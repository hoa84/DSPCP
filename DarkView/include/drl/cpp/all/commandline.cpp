...
unsigned CommandLine::argsize(char n) {
  return (unsigned) parameters[n].args.size();
}
...
unsigned CommandLine::argsize() {
  return (unsigned) parameters[' '].args.size();
}
...
unsigned long CommandLine::argul(char n, unsigned num) {
  return strtoul(parameters[n].args[num],NULL,0);
}
...
bool CommandLine::parse(int argc, char* argv[], Error *error) {
  ...
  while (argnum<argc) {
    ...
    for (unsigned i=0; i<m->second.num_args; i++) {
