...
namespace a {
  ...
  /// Replace any instance of \e source with \e target within the string \e s
  void str_replace(const char source, const char target, string &s);
  ...
  string itoa(int i);
  /// Returns string representation of long number
  string itoa(long i);
  ...
  string ftoa(double i);
    ...
    * No error checking for this prototype **/
  template <class t_type>
  inline void ascii_to(const string &value, t_type &newtype) {
  ...
  template <class t_type>
  inline void ascii_to(const string &value, t_type &newtype, Error &error) {
    istringstream in(value);
    in >> newtype;
    if (!in) {
      error.buffer() << "Error converting from string: " << value;
      error.addbuf(12,9,"Misc");
    }
  }
